    ...
    public partial class fImportGoods : Form
    {
        ...
        private void btn_nhaphang_suasl_Click(object sender, EventArgs e)
        {
            if (dgv_nhaphang_dshoadon.SelectedRows.Count > 0)
            {
                ...
                int sl_nhapvao = Convert.ToInt32(txt_ds_sl.Text) - sl_bandau;
        ...
        private void btn_nhaphang_dathang_Click(object sender, EventArgs e)
        {
            ...
            foreach(DataGridViewRow row in dgv_nhaphang_dshoadon.Rows) {
                masp = Convert.ToInt32(row.Cells["soluong"].Value);
                tensp =Convert.ToString(row.Cells["tensp"].Value.ToString());
                malsp = Convert.ToInt32(row.Cells["malsp"].Value);
                sl = Convert.ToInt32(row.Cells["soluong"].Value);
                dongiaban = Convert.ToInt32(row.Cells["dongiaban"].Value);
                dongianhap = Convert.ToInt32(row.Cells["dongianhap"].Value);
                data_provider.qlsanpham.Instance.sua(masp, tensp, malsp, sl, dongiaban, dongianhap);
